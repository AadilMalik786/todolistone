...
const TaskGen=(props)=>{
    ...
    return(
        ...
        <section className="bg-slate-400 h-[100vh] overflow-y-auto flex flex-col ">
            ...
            <textarea className="mt-10 resize-none h-20 w-[50%]" cols="30" rows="10" ref={myRef}></textarea>
            <button className="mt-5 w-[5%] bg-[red] flex items-center justify-center h-10 rounded-[10px] bg-purple-500"><GrAdd/></button>
             {editingTask ? <button className='py-2  flex justify-center bg-emerald-400 border-2 border-emerald-400  hover:bg-emerald-500 text-3xl w-[50%] text-white' onClick={()=>handleSave(myRef)}>SAVE</button> : ""}
            </form>
